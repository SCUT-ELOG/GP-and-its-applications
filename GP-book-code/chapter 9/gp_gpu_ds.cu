 ...
 */
 ...
 #define STACK_SIZE 50 //这个的取值也有待商榷
...
__global__ void execution_GPU(int* program, int* program_size, float* dataset, float* data_output, float* stack_cash){
    ...
    float data_stack[STACK_SIZE];//固定栈的大小，这样才能放到寄存器中.
    int data_idx = threadIdx.x + (blockIdx.x % 5) * (DATASET_SIZE / 5);
    while(data_idx < (DATASET_SIZE / 5) * (blockIdx.x + 1)){
        ...
        for (int j = 0; j < *program_size; ++j) {//每个线程对分配给自己的每个数据都遍历一次程序
            if (post[j * 2] == 0) {
                ...
                case 0: //'+'
                    ...
                    val1 = data_stack[stack_pointer - 1];
                    val2 = data_stack[stack_pointer - 2];
                    val_tmp = val1 + val2;
                    data_stack[stack_pointer - 2] = val_tmp;
                ...
                case 4: //'sqrt'
                    ...
                    data_stack[stack_pointer - 1] = val_tmp;
                    break;
                default:
                ...
                data_stack[stack_pointer] = dataset[data_idx * DATA_SIZE + post[j * 2 + 1]];//读入数据
                stack_pointer = stack_pointer + 1;
...
void evaluation_GPU(treeNode* candidate, int indiv_id, cudaStream_t* stream, Statistic& aver_post) {
    ...
    // cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>> post size: " << ' ' << size << endl;
    //  for (int i = 0; i < post.size(); ++i) {
    //      cout << post[i]->getLabel() << ' ';
    //  }
    //  cout << endl << endl;
    int* post_tmp = new int[size * 2];
    ...
    post_size_t[indiv_id] = size;
    cudaMemcpyAsync(post_size[indiv_id], &post_size_t[indiv_id], sizeof(int), cudaMemcpyHostToDevice, stream[indiv_id]);
    cudaMemcpyAsync(post_GPU[indiv_id], post_tmp, sizeof(int) * size * 2, cudaMemcpyHostToDevice, stream[indiv_id]);
...
cudaEventCreate(&start_);    //创建Event
cudaEventCreate(&stop_);
cudaEventRecord( start_,0);    //记录当前时间
     for(int i = 0; i < POPULATION; ++i){
        execution_GPU<<<5, THREAD_NUM, post_size_t[i] * 2 * sizeof(int), stream[i]>>>(post_GPU[i], post_size[i], input_GPU, output_GPU[i], stack_cash + i * THREAD_NUM * STACK_MAX_GM);
...
cudaEventRecord( stop_,0);    //记录当前时间
cudaEventSynchronize(start_);    //Waits for an event to complete.
cudaEventSynchronize(stop_);    //Waits for an event to complete.Record之前的任务
cudaEventElapsedTime(&time_elapsed,start_,stop_);    //计算时间差
printf("执行时间：%f(ms)\n",time_elapsed);
    ...
    //  for(int i = 0; i < POPULATION; ++i){
    //     //执行结果传输
    //     cudaMemcpy(ret_init, output_GPU[i], DATASET_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
    //      //获取适应度值
     ...
     for (int i = 0; i < GENERATION; ++i) {
        ...
        cudaDeviceSynchronize();
         for(int j = 0; j < POPULATION; ++j){
            ...
            //执行结果传输
            cudaMemcpy(ret[j], output_GPU[j], DATASET_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
